    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        map_create.setOnClickListener{
            ...
            if (reminderText.isEmpty()){
                toast("Please write reminder text!")
                return@setOnClickListener
            }

            if (selectedLocation == null){
                toast("Please set location!")
                return@setOnClickListener
            }
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == 123) {
            if (grantResults.isNotEmpty() && (grantResults[0] == PackageManager.PERMISSION_DENIED || grantResults[1] == PackageManager.PERMISSION_DENIED)) {
                toast("The reminder needs these permissions to function")
            }

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                if  (grantResults.isNotEmpty() && grantResults[2] == PackageManager.PERMISSION_DENIED) {
                    toast("This permission is needed for the app to function")
                }
            }
        }
        ...
        gMap.setOnMapClickListener { location:LatLng->
            with(gMap){
                ...
                try {
                    ...
                    city = adressList.get(0).locality
                    title = adressList.get(0).getAddressLine(0)
                } catch (e:Exception){
